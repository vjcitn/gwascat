...
\section{Formalizing disease traits with Disease Ontology}

The Disease Ontology project \cite{osborne} formalizes a vocabulary
...
will not be mappable.  However, consider
<<dogr>>=
