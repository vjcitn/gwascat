...
library(rtracklayer)
c17t = import(gffpath)
c17tg = as(c17t, "GRanges")
@

We make a Gviz DataTrack of the dsQTL scores.
<<lkgvt>>=
c17td = c17tg[ which(values(c17tg)$type == "Degner_dsQTL") ]
library(Gviz)
...
<<getstr>>=
c17ts = c17tg[ which(values(c17tg)$type == "Stranger_eqtl") ]
eqloc = AnnotationTrack(ranges(c17ts),  chrom="chr17", genome="hg19", name="Str eQTL")
...
if ("SNPlocs.Hsapiens.dbSNP.20111119" %in% installed.packages()[,1]) {
